...
VectorLexer::Token::Token(double val)
    : _type(DOUBLE_VAL), _val(val) {}

VectorLexer::Token::Token(std::string name)
    : _type(NAME), _val(name) {}

VectorLexer::Token::Token(unsigned long val)
    : _type(SIZE), _val(val) {}

VectorLexer::Token VectorLexer::Token::Comma() { return Token(',', SEPARATOR); }
VectorLexer::Token VectorLexer::Token::LeftPar() { return Token('(', START_SEQ); }
VectorLexer::Token VectorLexer::Token::RightPar() { return Token(')', FINISH_SEQ); }

VectorLexer::Token::operator double() const
{
    if (_type != DOUBLE_VAL) throw std::exception();
    return std::get<double>(_val);
}
VectorLexer::Token::operator std::string() const
{
    if (_type != NAME) throw std::exception();
    return std::get<std::string>(_val);
}
VectorLexer::Token::operator unsigned long() const
{
    if (_type != SIZE) throw std::exception();
    return std::get<unsigned long>(_val);
}
VectorLexer::Token::operator char() const
{
    if (_type != SEPARATOR && _type != START_SEQ &&
        _type != FINISH_SEQ)
        throw std::exception();
