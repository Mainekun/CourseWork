    ...
    for (auto i = tokens.begin(); i != tokens.end(); i++) {
        ...
        case START_SEQ:
            if (!(state & NAME_ENTERED))
                throw "Name was not defined";
            if (!(state & SIZE_ENTERED))
                throw "Size was not defined";
            if (state & START_ELEMENTS)
                throw "Extra \'(\'";
        ...
        case FINISH_SEQ:
            if (state != (NAME_ENTERED | SIZE_ENTERED | START_ELEMENTS | ELEMENT))
                throw "record is formed bad";
            if (size != elements.size())
                throw "Size does not match the amount of elements";
            result.push_back(VecLib::Vector(name, size, elements.data()));
            state = NONE;
            elements.clear();
            size = 0;
            name = "";
